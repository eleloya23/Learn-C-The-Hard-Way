...
{
  printf("%d %s %s %s\n", addr->id, addr->name, addr->lastname, addr->email);
}
                  ...
                  const char *lastname, const char *email)
{
  ...
  res = strncpy(addr->lastname, lastname, MAX_DATA);
  if(!res) die("Lastname copy failed");
...
Database_find(struct Connection *conn, const char *query)
{
  ...
  for(int i=0;i<MAX_ROWS;i++){
    ...
    if( 
        ((strcmp(query,addr->name)==0) 
            || (strcmp(query,addr->email)==0) 
                || (strcmp(query,addr->lastname)==0)) && 
            addr->set ){
      Address_print(addr);
      not_found = 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        ...
        case 's':
            if(argc != 7) die("Need id, name, lastname, email to set");
        ...
        case 'f':
            if(argc != 4) die("Need a name/lastname/email to find");
